...
# from .envs.states import JointState
from crowd_sim.envs.utils.state import JointState
from .logger import Logger, configure
from .type_aliases import TrainFreq, TrainFrequencyUnit
...
def joint_state_as_graph(state: JointState, device: th.device) -> dgl.DGLHeteroGraph:
    from .crowd_graph import CrowdNavGraph
    
    state_tensor = state.to_tensor(add_batch_size=False)
    return CrowdNavGraph(state_tensor).graph.to(device)
