...
SLAMTEC LIDAR Tutorial: <https://github.com/robopeak/lidar/wiki>
...
## Compile & Install lidar package

1. Clone lidar package from github

   Ensure you're still in the ros2_ws/src directory before you clone:

   ```bash
   git clone -b ros2 https://github.com/Slamtec/lidar.git
   ```

2. Build rpidar_ros package

   From the root of your workspace (ros2_ws), you can now build lidar package using the command:
...
4. Create udev rules for rplidar
   ...
   lidar running requires the read and write permissions of the serial device.
...
```bash
ros2 launch lidar view_rplidar_a1_launch.py
```

The command for RPLIDAR A2M7 is :

```bash
ros2 launch lidar view_rplidar_a2m7_launch.py
```

The command for RPLIDAR A2M8 is :

```bash
ros2 launch lidar view_rplidar_a2m8_launch.py
```

The command for RPLIDAR A2M12 is :

```bash
ros2 launch lidar view_rplidar_a2m12_launch.py
```

The command for RPLIDAR A3 is :

```bash
ros2 launch lidar view_rplidar_a3_launch.py
```

The command for RPLIDAR S1 is :

```bash
ros2 launch lidar view_rplidar_s1_launch.py
```

The command for RPLIDAR S1(TCP connection) is :

```bash
ros2 launch lidar view_rplidar_s1_tcp_launch.py
```

The command for RPLIDAR S2 is :

```bash
ros2 launch lidar view_rplidar_s2_launch.py
```

The command for RPLIDAR S2E is :

```bash
ros2 launch lidar view_rplidar_s2e_launch.py
```

The command for RPLIDAR S3 is :

```bash
ros2 launch lidar view_rplidar_s3_launch.py
```

The command for RPLIDAR T1 is :

```bash
ros2 launch lidar view_rplidar_t1_launch.py
```

The command for RPLIDAR C1 is :

```bash
ros2 launch lidar view_rplidar_c1_launch.py
```
