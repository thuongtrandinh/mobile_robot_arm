...
Tài liệu này mô tả chi tiết cách `controller` hoạt động trong ROS 2, từ lúc node được launch đến lúc trả lệnh điều khiển qua service `/ocp_plann`.

---

## 1) Mục tiêu của `controller`

`controller` là backend local planner kiểu **A* + smoothing + velocity planning + MPC**.
...
- Service cung cấp: `/ocp_plann` (`interfaces/srv/OcpLocalPlann`)
...
- `interfaces/JointState ob`
- `interfaces/Point sub_goal`
...
- `float64 al`
- `float64 ar`
- `interfaces/Point revised_goal`
- `interfaces/Point[] astar_path`
- `interfaces/ControlVar[] control_vars`
...
- Package đã build thành công (`controller`, `interfaces`)
...
ros2 launch controller ampcc_controller.launch.py
```
...
- [ ] `ros2 interface show interfaces/srv/OcpLocalPlann` đúng schema
